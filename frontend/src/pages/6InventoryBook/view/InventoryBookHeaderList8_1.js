...
// 반제품입고내역(보고용) - 품번별입고내역
const InventoryBookHeaderList81 = observer((props) => {
  ...
  const fInitGrid = async () => {
    ...
    props.DataProviderBinder(dataProvider, '8_1');
    props.GridViewBind(gridView, '8_1');
  };

  const fOnCurrentrowChanged = async (grid, oldrow, newrow) => {
    if (newrow >= 0) {
      props.RowChanged(grid, newrow, '8_1');
    }
  };

  const fOnCellClicked = (grid, index) => {
    if (props.currentIndex.current === index.dataRow && index.dataRow <= 0) {
      props.RowChanged(grid, index.dataRow, '8_1');
    }
  };
...
export default InventoryBookHeaderList81;
